...
const containsAnd = (arr) => {
  let arre=arr.filter(value=>value.includes("and"))
  return arre;
};
...
const oddValues = (arr) => {
  let arre=arr.filter(value=>value %2 !=0)
  return arre;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let arre =arr.filter(value => !(forbiddenValues.includes(value)))
      return arre;
    }
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  const arre = arr.filter(value => value.baseStat > minBaseStat)
  return arre;
};
...
const getStatName = (arr, minBaseStat) => {
  const arre = getBaseStatGreaterThan(arr, minBaseStat).map( item => item.stat.name)
  return arre;
};
...
const evenOddNumericValues = (arr) => {
  const arre = arr.filter(item => !isNaN(item));
  const e = arre.map(num => {
      if (num % 2 === 0){
        return "even";
    } else {
        return "odd";
