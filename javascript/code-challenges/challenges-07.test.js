...
const sortStarWarsCharacters = (starWarsArr) => {
  starWarsArr.sort((val1, val2) => {
    return val2.height - val1.height;
  });
  return starWarsArr;
};
...
const listFoods = (recipe) => {
  ...
  result = recipe.ingredients.map((item) => {
    if (item.split(' ', 5)[4] !== undefined) {
      return item.split(' ', 5)[2] + ' ' + item.split(' ', 5)[3] + ' ' + item.split(' ', 5)[4]
    } else if (item.split(' ', 5)[3] !== undefined) {
      return item.split(' ', 5)[2] + ' ' + item.split(' ', 5)[3];
    }
    else { return item.split(' ', 5)[2] }
